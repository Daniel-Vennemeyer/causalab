...
1. `.Codex/skills/research-session/CONVENTIONS.md` — research-session layout and active-session protocol.
2. `.Codex/skills/plan-experiment/ANALYSIS_GUIDE.md` — analysis dependency DAG, per-analysis decision matrix, auto-discovery rules.
...
Refuse to overwrite or sibling-add to an existing plan. See `.Codex/skills/research-session/CONVENTIONS.md` §One session, one plan.
...
> One session, one plan (see `.Codex/skills/research-session/CONVENTIONS.md`).
>
> Options:
...
- One session, one plan: refuse to write if the session already contains a plan-level artifact (see Step 0.5 and `.Codex/skills/research-session/CONVENTIONS.md` §One session, one plan). Never delete or overwrite an existing plan; route the user to a fresh session instead.
