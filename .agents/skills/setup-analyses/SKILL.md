...
It never edits anything under `causalab/`. Promotion to `causalab/analyses/` is a manual `git mv` step taken only after the prototype stabilizes (see `.Codex/skills/research-session/CONVENTIONS.md`).

## Required Reading

Before running this skill, read:

1. `.Codex/skills/research-session/CONVENTIONS.md` — research-session layout (especially "What goes in `code/`") and active-session protocol.
...
3. `.Codex/skills/plan-experiment/ANALYSIS_GUIDE.md` — the analysis dependency DAG, auto-discovery rules, and runner-YAML composition patterns.
...
- Read templates only from `.Codex/skills/setup-analyses/templates/`.
