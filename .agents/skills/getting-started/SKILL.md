...
description: Onboarding guide for new users. Explains the causalab codebase, how to set up tasks, what experiments are available, and how to run them with Codex. Use when the user asks "how do I use this?", "what can I do?", or needs a walkthrough.
---

# Getting Started with Causalab

This skill walks new users through the causalab codebase: what it does, how it's organized, how to set up tasks, and how to run experiments — all with Codex's help.
...
> 5. **How to run experiments with Codex** — The end-to-end workflow using skills
...
Most workflows start with a session skill: `/research-session` for experiments and analysis, `/development-session` for codebase work. AGENTS.md's mode detection usually invokes the right one for you automatically, but you can invoke either explicitly.
...
For the file-by-file breakdown of what each module is responsible for and the conventions every task must follow, see [`.Codex/skills/setup-task/instructions/task_package_layout.md`](../setup-task/instructions/task_package_layout.md). The `/setup-task` skill creates these files from a markdown spec.
...
### Using Codex to set up a task

Tell the user:

> You can ask Codex to set up a task for you. There are two ways:
>
...
> /setup-task
> ```
> Codex will walk you through creating the specification step by step — defining variables, templates, counterfactuals, and token positions.
>
...
## Section 5: How to Run Experiments with Codex

### The simple way

Tell the user:

> Just tell Codex what you want to investigate. For example:
>
> ```
> /run-experiment
> ```
>
> Codex will walk you through:
> 1. Selecting a task
...
| `/getting-started` | This guide — learn how to use causalab with Codex |
