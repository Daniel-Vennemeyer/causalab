...
The skill writes to `${SESSION_DIR}/code/methods/<name>/` — never to `causalab/`. Promotion to `causalab/methods/` is a manual `git mv` step taken only after the prototype stabilizes (see `.Codex/skills/research-session/CONVENTIONS.md`).
...
- Read templates only from `.Codex/skills/setup-methods/templates/`.
