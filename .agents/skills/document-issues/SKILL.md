...
Issues for the active session land in `${SESSION_DIR}/issues.md` (top level — see `.Codex/skills/research-session/CONVENTIONS.md`). The path is resolved via `agent_logs/.current`.
...
- **[CONFUSION]** Unclear which template file to use for token positions
  ...
  - Instructions mention `.Codex/skills/setup-task/templates/token_positions.py` but also reference patterns in existing tasks
