...
- **Linear (alphabet):** **no geodesic advantage exists** — even the *spline's own* geodesic loses to its straight line (1.22 > 0.94), an inversion robust to the corrected belief manifold. `w_manifold` *hurts* (0.71→1.24); the best steerer is plain `transition_centroid` (0.71). On a convex (linear) manifold the straight line already *is* the geodesic, so any added curvature (spline geodesic OR `w_manifold`) only pushes paths off it.
  - **The VAE actually BEATS the spline on the linear topology, on both axes.** Discovery: VAE `transition_centroid` isometry **0.73** vs the spline's own **0.058** (the spline's activation manifold barely aligns with behavior on alphabet). Steering: VAE `transition_centroid` distance **0.71** vs spline geometric 1.22 / linear 0.94. The spline's geodesic machinery assumes curvature a flat manifold lacks; the VAE's straight-latent interpolation matches it and wins. So the manifold-VAE is not merely a discovery-equivalent alternative — on non-cyclic topology it is the *better* steering method.
